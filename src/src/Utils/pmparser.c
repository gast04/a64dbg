...
/*
  print map minimal formatted
*/
void pmparser_print_minimal(procmaps_struct* map) {
  printf("%s\n", strlen(map->pathname)==0?"[anonym*]":map->pathname);
  printf("%p-%p\t%ld\t%s\n",
    map->addr_start, map->addr_end, map->length, map->perm);
}